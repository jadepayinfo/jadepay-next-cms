  ...
  const downloadTemplate = () => {
   ...
   const handleBroadcastImageUpload = async (
    ...
    const handleCreate = async () => {
      ...
      const customers = csvData.slice(1).filter(row => row.length >= 3 && row[0].trim());
  ...
  return (
    <>
       <div className="max-w-5xl mx-auto space-y-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-6">Send Notification From CSV File</h2>
                  
                  {/* Instructions */}
                  <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-6">
                    <h3 className="text-base font-semibold text-yellow-900 mb-3">📋 CSV File Format:</h3>
                    <ul className="space-y-2 text-sm text-yellow-900">
                      <li>• Column 1: <strong>phone</strong> - Phone number (e.g., 0812345678)</li>
                       <li>• Column 2: <strong>title</strong> - title text</li>
                      <li>• Column 3: <strong>message</strong> - Message text</li>
                      <li>• First row should be header: <code className="bg-yellow-100 px-2 py-1 rounded">phone,title,message</code></li>
                    </ul>
                    <button
                  ...
                  <section className="bg-gray-50 rounded-lg border border-gray-200 p-6">
                    <h3 className="text-base font-semibold text-gray-900 mb-4">1. Upload CSV File</h3>
                    ...
                    {!csvFile ? (
                      <label className="border-2 border-dashed border-gray-300 rounded-lg p-16 flex flex-col items-center justify-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition-all">
                        ...
                        <p className="text-gray-600 font-semibold text-lg mb-2">Click to upload or drag and drop</p>
                      ...
                      <div className="space-y-4">
                        ...
                        {csvPreview.length > 0 && (
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900 mb-3">Preview (First 3 rows):</h4>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                  {csvPreview.slice(1, 4).map((row, index) => (
                                    <tr key={index} className="hover:bg-gray-50">
                                      <td className="px-4 py-3 text-gray-900">{row[0]}</td>
                            ...
                            {csvData.length > 4 && (
                              <p className="text-xs text-gray-500 mt-2">... and {csvData.length - 4} more rows</p>
                            )}
                          </div>
                        )}
                        ...
                        <div className="bg-blue-50 border border-blue-400 rounded-lg p-4">
                          <p className="text-sm text-blue-900 font-semibold">
                            📊 Summary: Found {csvData.length - 1} customer{csvData.length - 1 > 1 ? 's' : ''} in the file
                          </p>
                          <p className="text-sm text-blue-800 mt-1">
                            The system will automatically send messages according to the file
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  {csvFile && (
                    <div className="bg-yellow-50 border border-yellow-400 rounded-lg p-4">
                      <p className="text-sm text-yellow-900">
                        ⚠️ Each customer will receive their own personalized message as specified in the CSV file
                      </p>
                    </div>
                  )}
                  ...
                  {csvFile && (
                    <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                      ...
                      >
                        {isSending ? "Sending..." : `📤 Send to ${csvData.length - 1} Customer${csvData.length - 1 > 1 ? 's' : ''}`}
                      </button>
                    </div>
                  )}
                </div>
